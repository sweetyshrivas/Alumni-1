...
const Navbar = ({ isAuthenticated, onLogout }) => {
  ...
  const handleSearch = debounce((e) => {
    ...
    if (searchQuery.trim()) {
      ...
      // Fetch search suggestions from an API or a local source
      setSearchSuggestions(['Example suggestion 1', 'Example suggestion 2']);
    }
  }, 300);
          ...
          {searchQuery && searchSuggestions.length > 0 && (
            <ul className="search-suggestions">
              {searchSuggestions.map((suggestion, index) => (
                <li key={index} className="search-suggestion-item">{suggestion}</li>
              ))}
            </ul>
          )}
        </form>
            ...
            <>
              ...
              {isProfileOpen && (
                <div className={`profile-dropdown ${isDarkMode ? 'dark-mode' : ''}`}>
                  ...
                  <a href="/change-password" aria-label="Change Password">Change Password</a>
